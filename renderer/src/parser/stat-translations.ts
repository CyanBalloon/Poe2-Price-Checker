...
function hasTradeIdForType(
  tradeIds: Record<string, string[]> | undefined,
  realType: ModifierType,
): boolean {
  if (!tradeIds) return false;
  ...
  if (specificTypes.includes(realType)) {
    if (ModifierType.Pseudo in tradeIds || ModifierType.Explicit in tradeIds) {
      return true;
    }
  }

  return realType in tradeIds || ModifierType.Explicit in tradeIds;
}
...
export function tryParseTranslation(
  ...
  for (const combination of _statPlaceholderGenerator(stat.string)) {
    ...
    if (
      !found ||
      (!hasTradeIdForType(found.stat.trade.ids, realType) &&
