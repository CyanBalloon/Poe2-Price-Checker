...
function parseRuneforged(item: ParserState) {
  ...
  if (
    ...
    if (item.baseType && _$REF?.ITEM_RUNEFORGED?.test(item.baseType)) {
      item.baseType = _$REF.ITEM_RUNEFORGED.exec(item.baseType)![1];
    }
    if (item.name && _$REF?.ITEM_RUNEFORGED?.test(item.name)) {
